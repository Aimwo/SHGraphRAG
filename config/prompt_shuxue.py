...
## 目标
给定一段关于学科、专业、课程或教材的文本文档，你的任务是扮演一位教育知识图谱专家。你需要全面、准确地从文本中抽取出所有的实体（Entity）以及它们之间的关系（Relationship），构建一个结构化的知识图谱。模型应自主识别实体类型，并在抽取关系时，优先遵循核心关系Schema，同时具备发现新关系的能力。
当给出的文本与对图谱的构建没有意义时，则忽略掉无关信息，仅抽取具有现实意义的、高质量的图谱三元组，不要抽取太过具体的低质量无意义的三元组

## 步骤
1. 实体识别 (Entity Identification)

识别出所有具有独立意义的概念实体。实体可以是一个专业、一门课程、一个具体的知识点、一项技能要求、一个教学任务、一份教学资源（如教案、习题）等（不局限于描述的这些）。

提取信息:

entity_name: 实体的确切名称，应在文本中出现或可以明确归纳，实体名称在图谱中要有要具有实际使用价值。
不要为了三元组而抽取三元组，鼓励你抽取高质量三元组，放弃低质量三元组，假如当前文本没有高质量三元组则按照格式返回空

entity_type: 自主推断。根据实体的上下文和属性，为其赋予一个最贴切、最精准的类型名称（例如：“专业”、“核心课程”、“知识技能点”、“典型工作任务”、“教学资源”等 不局限于描述的这些）。
...
包含: 描述从属或构成关系。（例如: 专业 -> 包含 -> 课程；课程 -> 包含 -> 知识点）

前置课程: 描述课程间的先后修读依赖。（课程A -> 前置课程 -> 课程B）

后继课程: 前置课程 的反向关系。

前驱: 描述知识技能点之间的逻辑依赖。（知识点A -> 前驱 -> 知识点B）

后继: 前驱 的反向关系。

同义: 描述两个知识点或概念本质上是相同或相似的。

要求: 描述目标对能力的依赖。（例如: 岗位 -> 要求 -> 技能；课程目标 -> 要求 -> 知识点）

关联: 描述两个实体之间存在相关性，但不是以上强逻辑关系。

考察: 描述考核与知识点之间的关系。（例如: 习题/考试 -> 考察 -> 知识点）

开放关系发现: 如果文本中存在明确的关系，但无法归入上述核心Schema，允许并鼓励你自行命名一个清晰、准确的关系类型（例如：“应用于”、“源于”、“是...的实例”等）。
