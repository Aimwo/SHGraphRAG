...
class GraphStructureBuilder:
    ...
    def create_relation_between_chunks(self, file_name: str, chunks: List, title: List) -> List[Dict]:
        """
        ...
        # MODIFICATION: Iterate over chunks and titles together using zip
        for i, (chunk, chunk_title) in enumerate(zip(chunks, title)):
            page_content = ''.join(chunk)
            current_chunk_id = generate_hash(page_content)
            position = i + 1

            # This logic for previous_chunk_id was slightly incorrect.
            # It should get the ID from the previously processed chunk in the list.
            previous_chunk_id = ""
            if i > 0:
                previous_chunk_id = lst_chunks_including_hash[-1]['chunk_id']
            ...
            chunk_data = {
                "id": current_chunk_id,
                "title": chunk_title,  # MODIFICATION: Add the title field
    ...
    def parallel_process_chunks(self, file_name: str, chunks: List, title: List, max_workers=None) -> List[Dict]:
        """
        ...
        if len(chunks) < 9:  # 对于小数据集，使用标准方法
            return self.create_relation_between_chunks(file_name, chunks, title)
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            title_batches.append(title[i:i + batch_size])
        
        # print(f"并行处理 {len(chunks)} 个块，每批次 {batch_size} 个，共 {len(chunk_batches)} 批次")
        
        # 为每个批次准备处理函数
        def process_chunk_batch(chunk_batch, title_batch, start_index):
            results = []
            current_chunk_id = ""
            batch_data = []
            relationships = []
            ...
            # 处理批次内的每个chunk
            for i, (chunk, chunk_title) in enumerate(zip(chunk_batch, title_batch)):
                ...
                if i > 0:
                    last_page_content = ''.join(chunk_batch[i - 1])
                    offset += len(last_page_content)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_batch = {
                executor.submit(process_chunk_batch, c_batch, t_batch, i * batch_size): i
                for i, (c_batch, t_batch) in enumerate(zip(chunk_batches, title_batches))
            }
