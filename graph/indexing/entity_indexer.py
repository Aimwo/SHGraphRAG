...
from config.settings import ENTITY_BATCH_SIZE, MAX_WORKERS as DEFAULT_MAX_WORKERS, ENTITY_VECTOR
...
class EntityIndexManager(BaseIndexer):
    ...
    def __init__(self, refresh_schema: bool = True, batch_size: int = 100, max_workers: int = 4):
        ...
        self.vector_type = ENTITY_VECTOR
    ...
    def create_entity_index(self, 
        ...
        # 创建新的向量索引
        try:
            # 从现有Neo4j数据库中提取指定节点标签（node_label）的文本属性（text_node_properties），向量化并存为属性embedding_node_property
    ...
    def _get_entity_texts_batch(self, entities: List[Dict[str, Any]], text_properties: List[str]) -> List[str]:
        ...
        for row in results:
            ...
            for prop in text_properties:
                ...
                if prop_text:
                    text_parts.append(prop_text.split("__")[0])
